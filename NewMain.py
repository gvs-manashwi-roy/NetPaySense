...
load_dotenv(override=False)  # override=False ensures that Hugging Face Secrets take priority over the .env file
...
async def predict(req: PredictionRequest):
    ...
    try:
        ...
        async def mock_bank_status():
            return None, None

        # 1. Community Check
        alert_task = asyncio.to_thread(check_nearby_failures, req.lat, req.lon)
        # 2. Tower Detection
        tower_task = asyncio.to_thread(tower.find_nearest_tower, req.lat, req.lon,
                                       OPENCELL_API_KEY) if OPENCELL_API_KEY else asyncio.sleep(0)
        # 3. Bank Status
        ...
        # 🔥 Filter out Cloud/Server ISPs (common on Hugging Face / AWS)
        CLOUD_ISPS = ["amazon", "google", "microsoft", "oracle", "digitalocean", "hetzner", "ovh", "cloudflare"]
        if live_operator and any(cloud.lower() in live_operator.lower() for cloud in CLOUD_ISPS):
            live_operator = None

        # Prioritize physical mobile towers found via OpenCellID
        final_operator = best_operator if best_operator != "Unknown" else (live_operator or "Airtel / Jio")
        ...
        response_data = {
            ...
            "metrics": {
                ...
                "operator": final_operator,  # 🟢 Badge shows current SIM (fallback to tower)
                "tower_count": nearest_tower_data.get("total_towers_found", 0) if isinstance(nearest_tower_data,
                                                                                             dict) else 0
            },
...
async def bank_status():
    ...
    try:
        ...
        for _, row in latest_df.iterrows():
            ...
            status = "DOWN" if "DOWN" in upi_raw else (
                "FLUCTUATING" if any(x in upi_raw for x in ["WARN", "FLUCT"]) else "UP")
            icon = "❌" if status == "DOWN" else ("⚠️" if status == "FLUCTUATING" else "✅")

            ts = row["timestamp"]
            stale = (current_time - ts).total_seconds() / 60 > 20 if pd.notna(ts) else True
            banks.append(
                {"bank": row["bank_name"], "status": status, "icon": icon, "timestamp": str(ts), "stale": stale})
...
# ----------- SUPABASE CLIENT -----------
def clean_env_var(var_name: str) -> str:
    val = os.getenv(var_name, "").strip()
    # Remove surrounding quotes if they exist (common issue in HF Secrets)
    if (val.startswith('"') and val.endswith('"')) or (val.startswith("'") and val.endswith("'")):
        val = val[1:-1].strip()
    return val


_supabase_url = clean_env_var("SUPABASE_URL")
_supabase_key = clean_env_var("SUPABASE_KEY")

print(f"DEBUG: SUPABASE_URL length: {len(_supabase_url)}")
print(f"DEBUG: SUPABASE_KEY length: {len(_supabase_key)}")

try:
    if _supabase_url and _supabase_key and "supabase.co" in _supabase_url:
        # Prevent Supabase/httpx from failing on HF proxy settings
        import os
        ...
        supabase = None
        print("Supabase credentials not set or invalid - feedback will be disabled.")
except Exception as e:
...
async def submit_feedback(req: FeedbackRequest):
    ...
    if supabase is None:
        raise HTTPException(status_code=503,
                            detail="Feedback storage not configured. Set SUPABASE_URL and SUPABASE_KEY in .env")
    try:
        record = {
            "lat": req.lat,
            "lon": req.lon,

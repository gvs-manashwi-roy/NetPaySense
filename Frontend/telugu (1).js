 ...
 * To use: Include this script in your index.html or run applyTELUGUTranslation() in the console.
 */
...
const TELUGU_TRANSLATIONS = {
    ...
    "📍 Fetch My Location": "📍 నా స్థానం పొందండి",
...
function applyTeluguTranslation() {
    ...
    if (typeof SIGNALS !== 'undefined') SIGNALS.length = 0, SIGNALS.push(...TELUGU_SIGNALS);
    if (typeof RECS !== 'undefined') {
        Object.keys(TELUGU_RECS).forEach(k => RECS[k] = TELUGU_RECS[k]);
    }
    if (typeof BANK_DATA !== 'undefined') {
        Object.keys(TELUGU_BANK_DATA).forEach(k => BANK_DATA[k] = TELUGU_BANK_DATA[k]);
    }
    ...
    window.submitFeedbackNew = function () {
        ...
        document.getElementById('fb-thanks-tip').textContent = TELUGU_FEEDBACK_TIPS[fbOutcome];
    ...
    window.obNext = function () {
        ...
        if (obIndex === slides.length - 1) {
            document.getElementById('ob-next-btn').textContent = TELUGU_TRANSLATIONS["Get Started ✓"];
        }
    };
    ...
    window.drawGauge = function (canvasId, labelId, tier) {
        ...
        const tierMap = {
            good: { end: Math.PI * 1.33, color: '#22c55e', label: TELUGU_TRANSLATIONS['Low Risk'] },
            mid: { end: Math.PI * 1.66, color: '#eab308', label: TELUGU_TRANSLATIONS['Moderate Risk'] },
            poor: { end: Math.PI * 2, color: '#ef4444', label: TELUGU_TRANSLATIONS['High Risk'] }
        };
        ...
        ctx.textAlign = 'left'; ctx.fillText(TELUGU_TRANSLATIONS['Low'], cx - r - 2, cy - 4);
        ctx.textAlign = 'right'; ctx.fillText(TELUGU_TRANSLATIONS['High'], cx + r + 2, cy - 4);
    ...
    window.renderBankCard = function (bank, titleId, rowsId, overallId) {
        ...
        if (titleEl) {
            const bankName = TELUGU_TRANSLATIONS[bank.name] || bank.name;
            titleEl.textContent = `${bankName} UPI సర్వర్ స్థితి`;
        }

        if (rowsEl) {
            rowsEl.innerHTML = bank.rows.map(r => {
                const label = TELUGU_TRANSLATIONS[r.label] || r.label;
                const status = TELUGU_TRANSLATIONS[r.status] || r.status;
        ...
        if (overallEl) {
            const overallText = TELUGU_TRANSLATIONS[bank.overall] || bank.overall;
    ...
    function translateElement(el) {
        ...
        if (el.nodeType === Node.TEXT_NODE) {
            ...
            if (TELUGU_TRANSLATIONS[text]) {
                el.textContent = el.textContent.replace(text, TELUGU_TRANSLATIONS[text]);
                return;
            }

            // Try matching without leading/trailing emojis or icons
            for (const key in TELUGU_TRANSLATIONS) {
                if (text.includes(key)) {
                    el.textContent = el.textContent.replace(key, TELUGU_TRANSLATIONS[key]);
                }
            }
            return;
        }
        ...
        // Handle placeholders
        if (el.placeholder && TELUGU_TRANSLATIONS[el.placeholder]) {
            el.placeholder = TELUGU_TRANSLATIONS[el.placeholder];
        }

        // Handle specific button texts and options
        if (el.id === 'ob-next-btn' && el.textContent.includes('Next')) {
            el.textContent = TELUGU_TRANSLATIONS["Next →"];
        }

        if (el.tagName === 'OPTION') {
            const trimmed = el.textContent.trim();
            if (TELUGU_TRANSLATIONS[trimmed]) {
                el.textContent = TELUGU_TRANSLATIONS[trimmed];
            }
        }
    ...
    if (dashSub && dashSub.textContent.includes('Check your current')) {
        dashSub.textContent = TELUGU_TRANSLATIONS["Check your current location's network reliability"];
    }
    ...
    obDesc.forEach(d => {
        ...
        if (TELUGU_TRANSLATIONS[txt]) d.textContent = TELUGU_TRANSLATIONS[txt];
    });

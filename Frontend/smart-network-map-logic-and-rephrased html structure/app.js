...
async function runAnalyzingWithCoords(name, lat, lng, btn) {
  ...
  setTimeout(async () => {
    const apiData = await fetchPrediction(lat, lng);
    if (!apiData) {
...
// 🚀 Move this AFTER validation
updateMapWithPrediction(apiData);
    ...
    // 🚫 Out of Karnataka
    if (apiData.status === "out_of_range") {
    ...
    currentLat = apiData.lat;
    currentLng = apiData.lon;

    document.getElementById('loc-name').textContent = name;
    document.getElementById('loc-coords').textContent =
      `${apiData.lat.toFixed(4)}, ${apiData.lon.toFixed(4)}`;

    // ✅ Replace SIGNALS with backend data
    const sig = {
      tier: apiData.tier,
      upi: apiData.upi,
      badge: apiData.badge
    ...
    list.innerHTML = `
      <li>📡 Best Network: <b>${apiData.best_network}</b></li>
      <li>💡 ${apiData.recommendation}</li>
      <li>📶 Type: ${apiData.type}</li>
      <li>⚡ Download: ${apiData.metrics.download}</li>
      <li>⏱ Latency: ${apiData.metrics.latency}</li>
    `;

    // Save history
    saveRecent(name, { lat, lng }, sig);
...
function initMap() {
  const render = () => {
    if (!map) {
      ...
      map.invalidateSize();
      map.setView([currentLat, currentLng], 16, { duration: 1 }); //replaced map.flyto->map.setView to prevent zoom reset
      mapMarker.setLatLng([currentLat, currentLng]).openPopup();
     if (betterNetworkCircle) {
  // Do nothing — backend controls circle now
}
    }
  };
  render();
...
function updateMapWithPrediction(apiData) {
  ...
  // 🔥 Wait if map not ready
  if (!map) {
    setTimeout(() => updateMapWithPrediction(apiData), 500);
    return;
  }

  const lat = apiData.lat;
  const lon = apiData.lon;

  // 🔥 Use backend dynamic location
  const betterLat = apiData.better_location?.lat || lat;
  const betterLon = apiData.better_location?.lon || lon;
  const distance = getDistance(lat, lon, betterLat, betterLon);

  // 🎨 Color logic
  let color =
    apiData.tier === "good" ? "#22c55e" :
    apiData.tier === "mid" ? "#f59e0b" :
    "#ef4444";
  ...
  mapMarker.bindPopup(`📍 You<br>${apiData.label}`).openPopup();
  ...
  // 📏 Dynamic radius
  let radius =
    apiData.tier === "good" ? 150 :
    apiData.tier === "mid" ? 100 :
    60;

  // 🔥 Draw dynamic circle at BEST LOCATION
  betterNetworkCircle = L.circle([betterLat, betterLon], {
    color: color,
    fillColor: color,
    fillOpacity: 0.25,
    radius: radius
  ...
  betterNetworkCircle.bindPopup("📶 Best Network Area").openPopup();

  // 🗺 Move map to better location
  map.setView([betterLat, betterLon], 16, { duration: 1 });
  let message = "";

if (distance < 20) {
  message = "✅ You are already in a good network area";
} else {
  message = `🚶 Move ${Math.round(distance)}m for better network`;
}

// Update UI
const adviceEl = document.getElementById("network-advice");
if (adviceEl) {
